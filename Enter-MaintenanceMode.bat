...
#------------------------------------------------------
errorMessage="*ERROR* - Unable to get host ID for '$hostToAdd'"
json=`cat host.get.json`
json=${json/HOSTTOADD/$hostToAdd}
json=${json/AUTHID/$auth}
result=`curl --silent --show-error --insecure --header $header --data "$json" $zabbixApiUrl`
hostId=`echo $result | jq -r '.result | .[0] | .hostid'`
if [ $hostId == null ]; then exit_with_error; fi
echo "HostId for '$hostToAdd': $hostId"


#------------------------------------------------------
# Add Host ID to group
# https://www.zabbix.com/documentation/2.0/manual/appendix/api/hostgroup/massadd
#------------------------------------------------------
errorMessage="*ERROR* - Unable to add $hostToAdd to maintenance group'"
json=`cat host.group.add.json`
...
echo "Successfully added $hostToAdd to Zabbix Maintenance Group"
